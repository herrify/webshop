    	...
    	//if ($res) {
            ...
            if (is_array($attach_exist) && count($attach_exist)>0 ) {
                ...
                if(is_array($attach_info) && count($attach_info)>0){
                    ...
                    /*
                    for($i=1;$i<11;$i++){
                        $attach_key = 'attach_'.$i;
                        if(empty($attach_info[$attach_key]) || !$attach_info[$attach_key] ){
                            $attach_info[$attach_key] = '';
                            //去掉 activity_userinfo 相关的信息
                            $attach_userinfo[$attach_key] = ''; //herry 2017-3-28 17:34
                        }    
                    }
                    
                    $attach['aid'] = $parames['aid'];

                    if(!isset($attach_info['username'])){
                        $attach_info['username'] = '';
                        $attach_userinfo['username'] = '';
                    }
                    if(!isset($attach_info['sex'])){
                        $attach_info['sex'] = '';
                        $attach_userinfo['sex'] = '';
                    }
                    if(!isset($attach_info['grade'])){
                        $attach_info['grade'] = '';
                        $attach_userinfo['grade'] = '';
                    }
                    if(!isset($attach_info['major'])){
                        $attach_info['major'] = '';
                        $attach_userinfo['major'] = '';
                    }
                    if(!isset($attach_info['tel'])){
                        $attach_info['tel'] = '';
                        $attach_userinfo['tel'] = '';
                    }
                    if(!isset($attach_info['qq'])){
                        $attach_info['qq'] = '';
                        $attach_userinfo['qq'] = '';
                    }
                    */
                    ...
                    foreach ($attach_info as $key_s => $value_s) {
                        ...
                        if($value_s){
                            ...
                            //$attach_userinfo[$key_s] = $value_s;
                        }
                    }

                    $rs = $db_activity_attach->where($attach)->save($attach_info);
                    //$rs = $db_activity_userinfo->where($attach)->save($attach_userinfo);
                }else{
                    $attach['aid'] = $parames['aid'];
...
error_log("res &&&& rs ++++++++++log++++++++++++".$rs."+++++".$res);
    	if(true){
    		...
    		if (is_array($activity_info)&&count($activity_info)>0) {
                ...
                //$data_return['information'] = $attachs;
                if($attachs){
                    unset($attachs['date']);
...
//var_dump($data);die;
        ...
        //获取表头信息
        $header = $db_activity_attach->where('aid=%d',$data['aid'])->find();
        ...
        //获取表信息
        $content = $db_activity_userinfo->where('aid=%d',$data['aid'])->order('addtime desc')->select();
        ...
        //获取活动名称
        $activity_title = $db_activity->where('id=%d',$data['aid'])->find();
        $title = $activity_title['activity_name'];

        $save_path = "/tmp/activity_export/"."{$title}-用户信息.xls";
        ...
        foreach ($header as $hk => $hv) {
            $hv = iconv('gb2312', 'utf-8', $hv);
            //var_dump($hv);
            $arr[] = $hv;
        }
        //$str_header = explode(',',$str_header);
        //unset($str_header[0]);
        //var_dump($arr);
        //$arr = array('asdf','asdfd','dsffe',1,1,3,4);
        //fputcsv($file, $arr);
        
        
        foreach ($content as $keys => $values) {
            foreach ($values as $key => $valuess) {
                $valuess = iconv('GBK', 'utf-8', $valuess);
                $arrs[] = $valuess;
            }
        }
        ...
        fclose($file);
//var_dump($paradms);die;
        $name='hello';
        $to=$data['email']?$data['email']:'1542964971@qq.com';
        $subject='【微群活动】“活动主题”报名表导出信息';
        $body='来自微群活动';
        //获取报名信息

        $aa = $this->think_send_mail($to, $name, $subject, $body,$attachment = array($save_path));
        if ($aa) {
            $reg['ret'] = 1;
            $reg['msg'] = 'ok';
            $reg['data'] = '';
...
/*
        ...
        $aa = $this->think_send_mail($to, $name, $subject, $body, $attachment = null);
        if ($aa) {
            $reg['ret'] = 1;
            $reg['msg'] = 'ok';
            $reg['data'] = '';
...
*/    
    ...
    function think_send_mail($to, $name, $subject = '', $body = '', $attachment = null){
        ...
        if($mail_type){//是 qq邮箱，用qq进行发送
            ...
            $mail->Username = "597247825@qq.com";// 发送方的163邮箱用户名
            $mail->Password = "gugroltawpxibfaj";// 发送方的邮箱密码，注意用163邮箱这里填写的是“客户端授权密码”而不是邮箱的登录密码！
            $mail->SMTPSecure = "tls";// 使用ssl协议方式
            $mail->Port = 25;// 163邮箱的ssl协议方式端口号是465/994

            $mail->setFrom("597247825@qq.com","微群活动");// 设置发件人信息，如邮件格式说明中的发件人，这里会显示为Mailer(xxxx@163.com），Mailer是当做名字显示
            $mail->addAddress($to,$name);// 设置收件人信息，如邮件格式说明中的收件人，这里会显示为Liang(yyyy@163.com)
            $mail->addReplyTo("597247825@qq.com","Reply");// 设置回复人信息，指的是收件人收到邮件后，如果要回复，回复邮件将发送到的邮箱地址
