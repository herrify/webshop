...
error_log("create_____log________".json_encode($attach_info));                
    ...
    public function ActivityInfo(){
        ...
        $users['is_involved'] = 1;
        $user_s = $db_activity_user->where($users)->order('id asc,addtime desc')->group('uid')->limit(5)->select();
        $user_arr = array();
        if($user_s){
...
*/
    ...
    public function JoinList(){
        ...
        $user_s = $db_activity_user->where($users)->order('id desc')->group('uid')->select();
        array_reverse($user_s);
        if($user_s){
            $join_list = array();
            foreach ($user_s as $key => $value) {
                if($value['is_involved'] ==1 || ($value['uid'] == $params['uid'])){
            ...
            //将当前用户放到第一位
            if(isset($current_user[$key]) && $current_user[$key]){
                array_unshift($join_list,$current_user[$key]);
                //array_splice($join_list, 0,1,$current_user);
